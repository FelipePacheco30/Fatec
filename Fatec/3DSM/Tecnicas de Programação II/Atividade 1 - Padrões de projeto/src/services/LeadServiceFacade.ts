    ...
    try {
      ...
      if (input.status) {
        ...
        if (input.status === 'Em negociação' && estagioParaCoerencia === 'Contato inicial') {
          throw new Error(
            'Status "Em negociação" só é permitido após avançar o estágio além de "Contato inicial".'
          );
        }
        if (
          (input.status === 'Finalizado com venda' ||
            input.status === 'Finalizado sem venda') &&
          estagioParaCoerencia !== 'Aguardando pagamento'
        ) {
          throw new Error(
            'Finalização só é permitida com estágio "Aguardando pagamento".'
          );
        }
